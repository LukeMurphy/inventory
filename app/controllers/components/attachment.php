...
class AttachmentComponent extends Object
{
	...
	/* You may define as many options as you like */
			'big'    => array(640, 640, false),
			'med'    => array(280, 280, true),
	...
	function upload_FS($data) {
		...
		$fileuploaddir  = WWW_ROOT.$this->config['upload_dir'];
		...
		$filenameParts = split('.'.$filetype, $filename);
		$filename = strtolower($filenameParts[0]);
		...
		if (is_uploaded_file($data['tmp_name'])) {
			...
			return array('name'=>$filename, 'path'=>$this->config['upload_dir'].'/'.$filename);
		}
	}
	 ...
	 */
	function thumbnail($tmpfile, $suffix, $maxw, $maxh, $crop = false) {
		// Make sure the required directory exist; create it if necessary
		//$suffix = $upload_dir;
		if($this->config['photos_dir'] !== '') {
			$upload_dir = WWW_ROOT.'attachments'.DS.$this->config['photos_dir'].DS.$this->config['upload_dir'];
			if (!is_dir($upload_dir)) mkdir($upload_dir, 0777, true);
		} else {
			$upload_dir = WWW_ROOT.$this->config['upload_dir'];
		}

		$file_name = end(explode(DS, $tmpfile)) . '_' . $suffix . ".jpg";
	 ...
	 * Example usage:
	 *	$this->Attachment->delete_files('fi
	 *le_name.jpg');
	 *
	 * Parameters:
	...
	function delete_files($filename) {
		...
		foreach ($this->config['images_size'] as $size => $opts) {
			$photo = WWW_ROOT.$this->config['upload_dir'].DS.$this->config['photos_dir'].DS.$size.DS.$filename;
	...
	function resizeImage($cType = 'resize', $tmpfile, $dstfolder, $dstname = false, $newWidth=false, $newHeight=false, $quality = 100) {
		...
		if ($newWidth or $newHeight) {
			...
			if(file_exists($dstimg)) {
				...
				//imagecopyresized($newImage, $oldImage, 0, 0, $startX, $startY, $applyWidth, $applyHeight, $oldWidth, $oldHeight);
